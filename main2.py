'''
Uwaga: póki co działa sensownie tylko dla grafów typu Graph-property - Node i Link to będzie chyba musiał być inny proces treningowy - to be continued
// TODO: zrobić żeby było
'''
...
# Open Graph Benchmark datasets (for different tasks (Graph, Node, Link), with different scale (small, medium, large) and different domains (molecular, knowledge, etc.)
OGB_GRAPH_PROPERTY_DATASETS = {'ogbg-molhiv', 'ogbg-molpcba', 'ogbg-ppa', 'ogbg-code2'}     # More info: https://ogb.stanford.edu/docs/graphprop/
OGB_NODE_PROPERTY_DATASETS = {'ogbn-arxiv', 'ogbn-products', 'ogbn-proteins', 'ogbn-mag'}   # More info: https://ogb.stanford.edu/docs/nodeprop/
OGB_LINK_PROPERTY_DATASETS = {'ogbl-collab', 'ogbl-ppa', 'ogbl-citation2'}                  # More info: https://ogb.stanford.edu/docs/linkprop/
...
def main():
    ...
    parser.add_argument('--train_mode', type=str, default="P",
    ...
    if args.dataset in OGB_GRAPH_PROPERTY_DATASETS:
        ...
        dataset_type = 'ogb-g'
    elif args.dataset in OGB_NODE_PROPERTY_DATASETS:
        from ogb.nodeproppred import PygNodePropPredDataset as Dataset, Evaluator
        dataset_type = 'ogb-n'
    elif args.dataset in OGB_LINK_PROPERTY_DATASETS:
        from ogb.linkproppred import PygLinkPropPredDataset as Dataset, Evaluator
        dataset_type = 'ogb-l'
    ...
    # # Special handling for OGB node-level datasets (single large graph with node splits)
    # if dataset_type == 'ogb-n':
    ...
    #     train_idx = to_list(split_idx['train'])
    #     val_idx = to_list(split_idx['valid'])
    #     test_idx = to_list(split_idx['test'])
    ...
    #     # Recreate classifier head if necessary (in case num_classes was unknown earlier)
    #     model.cls = torch.nn.Linear(model.cls.in_features, num_classes).to(device)

    #     y = data.y.view(-1).to(device)

    #     print(f"Running node-level training on {args.dataset} with {y.shape[0]} nodes")

    #     for epoch in range(args.epochs):
    #         model.train()
    #         optimizer.zero_grad()

    #         node_repr = model.gnn_model(data.x, data.edge_index)
    #         logits = model.cls(node_repr)
    #         out = torch.log_softmax(logits, dim=1)

    #         loss = nll_loss(out[train_idx], y[train_idx].long())
    #         loss.backward()
    #         optimizer.step()
    ...
    #             train_acc = (preds[train_idx] == y[train_idx].long()).float().mean().item()
    #             val_acc = (preds[val_idx] == y[val_idx].long()).float().mean().item()
    #             test_acc = (preds[test_idx] == y[test_idx].long()).float().mean().item()

    #         print(f"Epoch {epoch}: loss={loss.item():.4f}, train_acc={train_acc:.4f}, val_acc={val_acc:.4f}, test_acc={test_acc:.4f}")

    #     # After training, exit the script (node-level handled)
    #     return
    ...
    for idd in range(num_k):
        ...
        if dataset_type == 'ogb-l':     # link (ogb)
            split_edge = dataset.get_edge_split()
            train_data = split_edge["train"]
            val_data = split_edge["valid"]
            test_data = split_edge["test"]
        
        elif dataset_type == 'ogb-n':   # node (ogb)
            split_idx = dataset.get_idx_split()
            # convert tensors to plain Python lists of ints
        ...
        for epoch in range(args.epochs):
            ...
            for i, data in enumerate(train_loader):
                s = time.time()
                data = data.to(device)
                # ensure node features are float (avoid int64 vs float mismatch in GNN layers)
                if hasattr(data, 'x') and data.x is not None:
                    data.x = data.x.float()
                optimizer.zero_grad()
